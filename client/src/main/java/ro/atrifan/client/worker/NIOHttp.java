...
public class NIOHttp extends Thread {
    private static final Logger log = Logger.getLogger(NIOHttp.class);
    ...
    private static NIOHttp instance = null;
    public static NIOHttp getInstance() {
        if(instance == null) {
            try {
                instance = new NIOHttp();
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    public NIOHttp() throws IOException {
        ...
        synchronized (work) {
            if (add(work, timeout)) {
                try {
                    ...
                    log.error("NIO operation interrupted");
                }
            } else {
                log.error("Unable to add work to the NIOWorker");
            }
        }
    }
